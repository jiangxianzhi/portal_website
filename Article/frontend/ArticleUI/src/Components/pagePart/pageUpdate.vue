  ...
  <div>
    ...
    <div class="flex_container">
      <el-button
        ...
        class="toggleButton"
      >
        UpDate<el-icon class="el-icon--right"><Upload /></el-icon>
      </el-button>
    </div>
    <!-- 父节点 -->
    <!-- 头节点id == 0  -->
    <div style="display: flex; justify-content: center" class="fatherNode">
      <topNodeChoose :myselfId="page.id"></topNodeChoose>
...
export default {
  ...
  props: {
    page: {
      type: Object, // or the type of your data
  ...
  setup(props) {
    ...
    const title1 = ref("");
    //  初始化时就赋值了
    valueHtml.value = props.page.content;
    title1.value = props.page.title;
    store.commit("changeHeadNode", props.page.parentId);
    // 监听aritcle
    watch(
      () => props.page,
      (newPage, oldArticle) => {
    ...
    const updatePage = async () => {
      // 上传page时赋值，因为title1 watch不到初始化的变化
      Page.id = props.page.id;
      Page.parentId = store.state.choosedHeadNode;
      Page.title = title1.value;
      Page.content = valueHtml.value;
